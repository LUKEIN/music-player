KISSY.add(function (S, Node, Anim, XTemplate, IO, dd, ListTpl, TypeTpl) {
    ...
    function Player(container, options) {
        ...
        this.mList = [];
        this.lrc = [];
        this.defaltImg = 'img/default.jpg';
        this.currentPlay = 0;
        this.animStyle = {
    ...
    Player.prototype.handleTimeUpdate = function (self) {
        ...
        return function () {
            ...
            handler.css('left', handPos-1  + '%');//为啥要减一   handler.css('left', handPos-1  + '%');
            if (this.ended)
        ...
        toggleBtn.on('click', function () {
            if (self.audio.paused) {
                ...
                if (self.mList.length === 0)
                    return;
                self.audio.play();
                self.animCD.isPaused() ? self.animCD.resume() : self.animCD.run();//有动画暂停就继续当前动画，没有的话就开始当前动画
                $(this).css('background-position', '-292px -90px');//sprite

            } else {
                self.audio.pause();
                self.animCD.pause();
                $(this).css('background-position', '-102px -44px');
            }
            ;
        });
    ...
    Player.prototype.musicList = function () {
        ...
        listEl.delegate('click', '.m-operate .m-delete', function (ev) {
            ...
            if (self.mList.length === 0) {
                self.animCD.pause();
                $('.toggle').css('background-position', '-102px -44px');
            }
            ev.halt();
        });
    ...
    Player.prototype.initTepeWrap = function (data) {
        var self = this,
            nav = $('.type-nav'),
            navItem = nav.all('a'),
        ...
        searchInput.on('valuechange', function (ev) {
            ...
            new IO({
                ...
                success: function (data) {
                    var mulStr = '',
                        resultCount = 0;
                    if (data["song"].length === 0 && data["singer"].length === 0 && data["album"].length === 0) return;
                    for (var i = 0; i < data["song"].length; i++) {
                        if (resultCount >= 5)
                            break;
                        mulStr += '<li class = "hint-item"><span class = "s-info">' + data["song"][i].songName + '</span><span class = "s-type">歌曲</span></li>';
                        resultCount++;
                    }
                    for (var i = 0; i < data["singer"].length; i++) {
                        if (resultCount >= 5)
                            break;
                        mulStr += '<li class = "hint-item"><span class = "s-info">' + data["singer"][i].singerName + '</span><span class = "s-type">歌手</span></li>';
                        resultCount++;
                    }
                    for (var i = 0; i < data["album"].length; i++) {
                        if (resultCount >= 5)
                            break;
                        mulStr += '<li class = "hint-item"><span class = "s-info">' + data["album"][i].albumName + '</span><span class = "s-type">专辑</span></li>';
                        resultCount++;
                    }
